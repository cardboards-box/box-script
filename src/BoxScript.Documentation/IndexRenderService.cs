    ...
    IScriptEngineSettings _settings) : IIndexRenderService
{
    ...
    public static IEnumerable<string> Enum(string name, Comments desc)
    {
        ...
        yield return $"declare class {name} {{";
    ...
    public async Task WriteIndex(StreamWriter writer, string moduleName)
    {
        ...
        await writer.WriteLineAsync("// This file is auto-generated from the loaded modules");
        await writer.WriteLineAsync("// You should avoid modifying this file directly");
        await writer.WriteLineAsync($"// Generated: {DateTime.UtcNow:yyyy-MM-dd HH:mm} UTC");

        if (_settings.Enums.Any())
        {
            foreach (var type in _settings.Enums)
            {
                types.Add(type);
                var comment = _classes.Enum(type);
                await WriteEnum(writer, comment, type.Name, indent);
            }
            await writer.WriteLineAsync();
        }
